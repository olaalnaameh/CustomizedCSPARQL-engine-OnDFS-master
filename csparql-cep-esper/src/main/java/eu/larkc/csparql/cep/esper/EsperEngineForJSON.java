...
import eu.larkc.csparql.cep.api.JSONSnapshot;
import eu.larkc.csparql.cep.api.JSONStream;
import eu.larkc.csparql.common.config.Config;

public class EsperEngineForJSON implements CepEngineForJSON {
...
//	private Collection<RdfStream> streams = null;
	private Collection<JSONStream> streams = null;
...
//	private List<RdfQuadruple> quadAtBoundary = new ArrayList<RdfQuadruple>();
	private List<JSONData> JSONAtBoundary = new ArrayList<JSONData>();

	

	//protected final Logger logger = LoggerFactory.getLogger(EsperEngine.class);
	protected final Logger logger = LoggerFactory.getLogger(EsperEngineForJSON.class);
...
public Collection<JSONStream> getAllRegisteredStreams() {
		return streams;
	}
	...
	public void initialize() {
		...
		this.streams = new ArrayList<JSONStream>();
	...
	public void setUpInjecter(int queueDimension) {
		...
		if (queueDimension == 0) {
			...
			this.queue = new ArrayBlockingQueue<JSONData>(queueDimension);


			//EsperInjecter esj = new EsperInjecter(queue, this);
			EsperJSONInjecter esj = new EsperJSONInjecter(queue, this);			
...
public void registerStream(final JSONStream p) {
		String un = p.uniqueName();
		// this.epService.getEPAdministrator().getConfiguration().addImport(RdfQuadruple.class);
		/*
		 * there are two problems need to be fixed with using external time stamp. 1. how to get range and step here 2. how to take care the steps of mutli queries. like, there is an empty slide.
		 */
		this.epService.getEPAdministrator().getConfiguration().addEventType(un, JSONData.class);
		p.addObserver(this);
		this.streams.add(p);
		System.out.println("EsperEngineForJSON.registerStream(final JSONStream p)- Stream Registered.");
	}
...
public RdfSnapshot registerQuery(final String query, final String id) {
		...
		//final QueryListener l = new QueryListener(id);
		final FactorizationListenerJSON l = new FactorizationListenerJSON(id);
		stmt.addListener(l);
		return l;
	}
...
public void unregisterStream(final JSONStream stream) {
		this.streams.remove(stream);
	}
...
@Override
	public void update(Observable o, Object arg) {
		...
		//RdfStream s = (RdfStream) o;
		JSONStream s = (JSONStream) o;
		//RdfQuadruple q = (RdfQuadruple) arg;
		JSONData q = (JSONData) arg;
		q.setStreamName(s.getIRI());
...
@Override
	public Long setCurrentTimeAndSentEvent(JSONData q) {
		long inputTime = q.getTimestamp();
		if (!isSysTimeInit) {
			// Using the beginning of the current tick as the the window boundary
			long initTime = inputTime / this.timeStampTick * this.timeStampTick;
		...
		while (inputTime > (this.currentSystemLastTickTime + this.timeStampTick)) {
			if (this.JSONAtBoundary.size() > 0) {
				long boundayTime = this.JSONAtBoundary.get(0).getTimestamp();
				for (JSONData tempQ : this.JSONAtBoundary) {
					this.epService.getEPRuntime().sendEvent(tempQ);
				}
				this.JSONAtBoundary = new ArrayList<JSONData>();
				this.currentSystemTime = boundayTime;
				this.currentSystemLastTickTime += this.timeStampTick;
				this.epService.getEPRuntime().sendEvent(new CurrentTimeEvent(this.currentSystemTime));
				this.currentSystemTime += 1;
